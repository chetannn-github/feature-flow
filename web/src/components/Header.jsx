...
import { Cross, Flag, Menu, X } from "lucide-react";
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    document.getElementsByTagName("main")[0].addEventListener('click', handleDocumentClick);
    return () => {
        document.getElementsByTagName("main")[0].removeEventListener('click', handleDocumentClick);
    };
  },[]); 
          ...
          <div className="md:hidden py-4 border-t border-border">
            <nav className="flex flex-col gap-4">
              <div className="flex flex-col gap-2 pt-4 border-t border-border">
                <div className="flex gap-3 items-center" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>
                  <ThemeToggle />
                  <div>Toggle Theme</div>
                </div>
                

              {!auth?.token && <Link to={"/sign-in"} onClick={closeMenu}>
              ...
              {!auth?.token &&<Link to={"/sign-up"} onClick={closeMenu}>
              ...
              {auth?.token &&<Link to={"/dashboard"} className="text-muted-foreground hover:text-foreground transition-colors" onClick={closeMenu}>
                    <Button variant="ghost"> Go to Dashboard</Button>
                </Link>
              }

              {auth?.token && <div className="w-full" onClick={() => dispatch(logout())} onClick={closeMenu}>
